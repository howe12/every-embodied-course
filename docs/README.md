<div align="center">

  # Every Embodied Course
  
  ## ROS2机器人仿真课程

  > 本课程围绕 [datawhalechina/every-embodied](https://github.com/datawhalechina/every-embodied) 开展
  > 
  > 仅需Python基础，从0构建自己的具身智能机器人；从0逐步构建VLA/OpenVLA/SmolVLA/Pi0，深入理解具身智能

  <p align="center">
    📌 课程主页 · ✨ 学习路线 · 🤖 实践项目
  </p>

  <p align="center">
      <a href="https://github.com/howe12/every-embodied-course/stargazers" target="_blank">
          <img src="https://img.shields.io/github/stars/howe12/every-embodied-course?color=0052cc&style=for-the-badge" alt="Stars"></a>
      <a href="https://github.com/howe12/every-embodied-course/network/members" target="_blank">
          <img src="https://img.shields.io/github/forks/howe12/every-embodied-course?color=0052cc&style=for-the-badge" alt="Forks"></a>
  </p>
  
  <p align="center">
    🔗 <a href="https://howe12.github.io/every-embodied-course/" target="_blank">在线访问课程</a>
  </p>
</div>

---

## 🚀 快速开始

```bash
# 1. 克隆仓库
git clone https://github.com/howe12/every-embodied-course.git
cd every-embodied-course

# 2. 安装ROS2 (以Humble为例)
sudo apt update
sudo apt install ros-humble-desktop

# 3. 运行第一个示例
ros2 run turtlesim turtlesim_node
```

## 📖 学习路径

本教程分为多个阶段，带你逐层深入具身智能：

| 阶段 | 核心技能 | 产出 |
| :--- | :--- | :--- |
| **第一阶段** | 机器人基础、ROS2入门、Linux操作 | 搭建ROS2开发环境 |
| **第二阶段** | 工作空间、构建工具、常用工具 | 独立开发ROS2功能包 |
| **第三阶段** | 硬件控制、传感器选型 | 机器人硬件实战 |
| **第四阶段** | 计算机视觉、深度学习 | 视觉感知能力 |
| **第五阶段** | 强化学习、策略优化 | 智能决策系统 |
| **第六阶段** | VLA大模型、前沿研究 | 具身智能进阶 |

---

## 🗺️ 学习地图

### 01-具身智能概述
- 01-1 机器人发展历史
- 01-2 机器人系统组成
- 01-3 ROS发展历程
- 01-4 ROS2系统架构
- 01-5 ROS2安装
- 01-6 Linux操作基础

### 02-机器人基础和控制
- 02-1 ROS2工作空间
- 02-2 Colcon构建工具
- 02-3 开发工具链
- 02-4 ROS2常用工具
- 02-5 版本控制

### 03-机器人硬件、lerobot及地瓜RDK-X5开发板控制教程
- 03-1 RDKX5超新手入门教程
- 03-2 传感器选型与数据采集
- 03-3 执行器原理与选型
- 03-4 LeRobot遥操作

### 04-具身场景的计算机视觉、3D重建
- 04-1 SAM和深度估计
- 04-2 抓取注意力热图

### 05-具身场景的深度和强化学习
- 05-1 多机器人搬运家具强化学习

### 06-策略抓取或抓取VLA
- 06-1 SmolVLA-LIBERO
- 06-2 OpenVLA复现
- 06-3 RT系列论文解读

### 07-机器人操作、运动控制
- 07-1 春晚舞蹈机器人复刻

### 08-具身导航及VLN
- 08-1 ETPNav-VLN导航复现

### 09-具身智能数据及评估基准benchmark
- 09-1 LIBERO
- 09-2 SimplerENV

### 10-具身智能其他仿真工具及仿真前沿
- 10-1 GenieSim3配置

### 12-具身智能面试问题汇总
- 12-1 具身智能面试问题

### 13-其他前沿项目复现
- 13-1 无人机多模态大模型

---

## 🤝 贡献者名单

| 姓名 | 职责 | 简介 | GitHub |
| :--- | :--- | :--- | :--- |
| **霍海杰** | ROS2课程维护 | ROS2机器人仿真课程维护 | [@howe12](https://github.com/howe12) |
| **Ethan-Chen-plus** | 项目负责人 | 中国科学院大学 | [@Ethan-Chen-plus](https://github.com/Ethan-Chen-plus) |
| **YYPro** | 第1、2、3、4章贡献者 | 中国科学院大学 | [@YYpro](https://github.com/Suibian-YY-pro) |
| **李昀迪** | 第2、8、13章贡献者 | 北京科技大学 | [@muzilyd](https://github.com/muzilyd) |
| **张天一** | 第8章贡献者 | 北京工业大学 | [@GodoneZ](https://github.com/GodoneZ) |
| **陈可为** | 第5、6、7、9、10章贡献者 | 中国科学院大学 | [@Ethan-Chen-plus](https://github.com/Ethan-Chen-plus) |

---

## 📄 LICENSE

本项目文档与教程内容采用 **Creative Commons Attribution 4.0 International (CC BY 4.0)** 许可协议。
